...
import { DeletableHeadersAPI, DeleteHeadersAPI } from '../../../utils/APIcall';
import { useSelector } from 'react-redux';

export const DeleteUserModel = ({ datares, isOpen, onClose, onSubmit }) => {


    const token = useSelector((state) => state.token);

    const handleDelete = async (ID) => {
        console.log("Delete Method runing",ID)

        try {
            const res = await DeletableHeadersAPI(token, ID)
            ...
            if (error.response) {
                if (error.response.status === 401) {
                  setPasswordApiErr("Incorrect Password");
                    ...
                    showToastMessage_error(error.response.message);
                ...
                showToastMessage_error("Internal server error 404");
              }
        }
        finally {
            window.location.reload();
    ...
    return (
        <div className="fixed inset-0 z-50 overflow-auto bg-gray-900 bg-opacity-75 flex">
            <div className="relative p-8 bg-white w-full max-w-md m-auto flex-col flex rounded-lg">
                ...
                <h2 className="text-2xl font-semibold mb-4">Delete user ads</h2>

                {datares.length === 0 ? (
                    <div className='flex justify-center my-3'>No user found</div>
                ) : (
                    datares.map((dataMap) => (
                        <div key={dataMap._id} className='flex justify-between py-2 mb-1 hover:bg-slate-500'>
                            <span>{dataMap.username}</span>
